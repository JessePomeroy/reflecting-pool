...
describe("Contact page draft preview grant", () => {
	...
	it("rejects tampering, another tenant, and expiry", async () => {
		...
		await expect(
			verifyContactPagePreviewGrant(SECRET, `${token.slice(0, -1)}x`, "zippymiggy.com", NOW),
		).resolves.toBeNull();
		await expect(
			verifyContactPagePreviewGrant(SECRET, token, "other.example", NOW),
		).resolves.toBeNull();
		await expect(
			verifyContactPagePreviewGrant(
				SECRET,
				token,
				"zippymiggy.com",
				NOW + CONTACT_PAGE_PREVIEW_TTL_SECONDS * 1000,
			),
		).resolves.toBeNull();
	});
});

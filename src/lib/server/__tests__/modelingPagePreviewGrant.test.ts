...
describe("Modeling page draft preview grant", () => {
	...
	it("rejects tampering, another tenant, and expiry", async () => {
		...
		await expect(
			verifyModelingPagePreviewGrant(SECRET, `${token.slice(0, -1)}x`, "zippymiggy.com", NOW),
		).resolves.toBeNull();
		await expect(
			verifyModelingPagePreviewGrant(SECRET, token, "other.example", NOW),
		).resolves.toBeNull();
		await expect(
			verifyModelingPagePreviewGrant(
				SECRET,
				token,
				"zippymiggy.com",
				NOW + MODELING_PAGE_PREVIEW_TTL_SECONDS * 1000,
			),
		).resolves.toBeNull();
	});
});

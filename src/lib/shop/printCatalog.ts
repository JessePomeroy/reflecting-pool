/**
 * Print Catalog — LumaPrints lookup tables for paper, size, and cost metadata.
 *
 ...
 *
 * This module mirrors the Studio's `schemaTypes/shared/printCatalog.ts`
 * contract. Keep both modules in sync when adding papers, sizes, or costs.
 */
...
export function getWholesaleCost(paperSlug: string, sizeSlug: string): number | null {
	if (isCanvasPaper(paperSlug)) {
		...
		const thickness = paperSlug.endsWith("-rolled") ? "rolled" : paperSlug.split("-").at(-1);
		return thickness ? getCanvasWholesaleCost(thickness, sizeSlug) : null;
	}

...
import { verifyHomepageQuotePreviewGrant } from "$lib/server/homepageQuotePreviewGrant";
...
vi.mock("$convex/api", () => ({
	api: { content: { getHomepageQuoteEditorState: "content.getHomepageQuoteEditorState" } },
}));

import { POST } from "../preview/homepage-quote/+server";

function request(draftRevisionId = "revision-1") {
	return new Request("https://zippymiggy.com/api/admin/preview/homepage-quote", {
		method: "POST",
...
describe("POST /api/admin/preview/homepage-quote", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		verifyAdmin.mockResolvedValue(true);
		getAuthenticatedConvex.mockResolvedValue({ query });
		query.mockResolvedValue({
			draft: { revisionId: "revision-1", payload: { text: "Draft", attribution: "Artist" } },
		});
	});
	...
	it("issues an HttpOnly path-scoped grant for the exact current draft", async () => {
		...
		expect(query).toHaveBeenCalledWith("content.getHomepageQuoteEditorState", {
			siteUrl: "zippymiggy.com",
		});
		const [name, token, options] = input.cookies.set.mock.calls[0];
		expect(name).toBe("cms_homepage_quote_preview");
		...
		await expect(
			verifyHomepageQuotePreviewGrant(SECRET, token, "zippymiggy.com"),
		).resolves.toMatchObject({ draftRevisionId: "revision-1" });
		await expect(response.json()).resolves.toEqual({ previewUrl: "/preview/homepage" });
	});

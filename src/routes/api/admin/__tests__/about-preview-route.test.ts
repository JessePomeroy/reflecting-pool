...
import { verifyAboutPagePreviewGrant } from "$lib/server/aboutPagePreviewGrant";
...
vi.mock("$convex/api", () => ({
	api: { content: { getAboutPageEditorState: "content.getAboutPageEditorState" } },
}));

import { POST } from "../preview/about/+server";

function request(draftRevisionId = "revision-1") {
	return new Request("https://zippymiggy.com/api/admin/preview/about", {
		method: "POST",
...
describe("POST /api/admin/preview/about", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("issues an HttpOnly path-scoped grant for the exact current draft", async () => {
		...
		expect(query).toHaveBeenCalledWith("content.getAboutPageEditorState", {
			siteUrl: "zippymiggy.com",
		});
		const [name, token, options] = input.cookies.set.mock.calls[0];
		expect(name).toBe("cms_about_page_preview");
		expect(options).toMatchObject({
			path: "/preview/about-page",
			httpOnly: true,
			secure: true,
			sameSite: "lax",
			maxAge: 600,
		...
		await expect(
			verifyAboutPagePreviewGrant(SECRET, token, "zippymiggy.com"),
		).resolves.toMatchObject({ draftRevisionId: "revision-1" });
		await expect(response.json()).resolves.toEqual({ previewUrl: "/preview/about-page" });
	});

...
import { verifyModelingPagePreviewGrant } from "$lib/server/modelingPagePreviewGrant";
...
vi.mock("$convex/api", () => ({
	api: { content: { getModelingPageEditorState: "content.getModelingPageEditorState" } },
}));

import { POST } from "../preview/modeling/+server";

function request(draftRevisionId = "revision-1") {
	return new Request("https://zippymiggy.com/api/admin/preview/modeling", {
		method: "POST",
...
describe("POST /api/admin/preview/modeling", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("issues an HttpOnly path-scoped grant for the exact current draft", async () => {
		...
		expect(query).toHaveBeenCalledWith("content.getModelingPageEditorState", {
			siteUrl: "zippymiggy.com",
		});
		const [name, token, options] = input.cookies.set.mock.calls[0];
		expect(name).toBe("cms_modeling_page_preview");
		expect(options).toMatchObject({
			path: "/preview/modeling-page",
			httpOnly: true,
			secure: true,
			sameSite: "lax",
			maxAge: 600,
		...
		await expect(
			verifyModelingPagePreviewGrant(SECRET, token, "zippymiggy.com"),
		).resolves.toMatchObject({ draftRevisionId: "revision-1" });
		await expect(response.json()).resolves.toEqual({ previewUrl: "/preview/modeling-page" });
	});
